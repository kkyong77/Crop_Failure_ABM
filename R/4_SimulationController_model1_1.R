## using cumulative precipitation to compute the probabilities of the crop planting 
...
source("R/AgentClass_model1.R")  # Your Agent class with simulate_year method
...
for (yr in sim_years) {
      ...
      if (crop_pred %in% c("C", "B")) {
       ...
       precip_df <- agent$precip_ts %>%
        ...
        for (w in weeks_to_check) {
          p1 <- precip_df %>% filter(week == w - 1) %>% pull(weekly_precip)
          p2 <- precip_df %>% filter(week == w) %>% pull(weekly_precip)
          p1 <- ifelse(length(p1) == 0, 0, p1)
          p2 <- ifelse(length(p2) == 0, 0, p2)
          twoprecip <- p1 + p2
          prob_plantable <- 1 / (1 + exp(-(beta_0 + beta_1 * (twoprecip - b_p))))
  ...
  for (crop_i in crops) {
    ...
    # Sort by probability and assign evenly to weeks
    #crop_df <- crop_df %>%
    ...
    total_acres_crop <- crop_df %>%
      distinct(FBndID, .keep_all = TRUE) %>%                    # Keep only unique FBndID rows
      summarise(total_acres = sum(acres, na.rm = TRUE),        # Sum their acres
                .groups = "drop") %>%
      pull(total_acres)                                         # Extract the numeric value directly
    
    
    max_weekly_acres <- total_acres_crop / num_weeks
    ...
    for (w in weeks) {
      week_df <- crop_df %>%
        ...
        filter(cumsum_acres <= max_weekly_acres)
      
      message("Week ", w, " — Selected: ", nrow(week_df))
...
# --- Save Outputs ---
write_csv(crop_hist_all, "model/output/model1_simulated_agent_crop_history.csv")
saveRDS(agents, "model/output/model1_simulated_agents_list.rds")
...
## planted area per weekl for corn and soybean by year 
...
ggplot(crop_c_b_cum, aes(x = plant_week, y = cum_pct, color = crop)) +
  geom_line(size = 1) +
...
ggplot(crop_c_b_cum, aes(x = plant_week, y = cum_acres, color = crop)) +
  geom_line(size = 1) +
  geom_point(size = 2, alpha = 0.7) +
  facet_wrap(~ year) +
  labs(
    title = "Cumulative Planted area by Week",
    x = "Plant Week",
    y = "Cumulative planted area (acres)",
    color = "Crop"
  ) +
  theme_minimal() +
  theme(
...
ggplot(combined_df %>% filter(year >= 2010), 
...
crop_area_percent %>%
  filter(year %in%sim_years) %>%
  ggplot(aes(x = year, y = percent, color = crop)) +
