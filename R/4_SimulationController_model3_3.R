## consider the cumulative moving two weekly climate moisture and insurance coverage and RCI
library(dplyr)
library(lubridate)
library(readr)
library(ranger)
...
source("R/AgentClass_model5.R")  # Your Agent class with simulate_year method
...
set.seed(42)
sampled_agents <- agent_profile  %>% sample_n(100)
  ...
  for (fbnd_id in names(agents)) {
    ...
    if (!any(is.na(c(prev_crop1, prev_crop2, prev_crop3))) && !is.null(agent$moist_ts)) {
      ...
      if (crop_pred %in% c("C", "B")) {
        ...
        cat("twomoist:", twomoist, "\n")
        cat("agent insurance_coverage:", agent$insurance_coverage, "\n")
        cat("agent rotation_index:", agent$rotation_index, "\n")
        cat("beta_0:", beta_0, "beta_1:", beta_1, "b_d:", b_d, "\n")
        cat("beta_2:", beta_2, "b_in:", b_in, "\n")
        cat("beta_3:", beta_3, "b_rci:", b_rci, "\n")
        ...
        for (w in weeks_to_check) {
          ...
          twomoist <- p1 + p2
          prob_plantable <- 1 / (1 + exp(-(beta_0 + beta_1 * (twomoist - b_d)+beta_2*(agent$insurance_coverage-b_in)+beta_3*(agent$rotation_index-b_rci))))
...
# --- Save Outputs ---
write_csv(crop_hist_all, "model/output/model5_1_simulated_agent_crop_history.csv")
saveRDS(agents, "model/output/model5_1_simulated_agents_list.rds")
...
ggsave("model/figures/model5/model5_1_crop_planting_comparison_sim_obs_2018_2019.jpg",
       plot = combined_plots,
...
ggsave("model/figures/model5/model5_1_crop_percent_comparison_sim_obs_2018_2019.jpg",
       plot = p3,
       width = 14, height = 6, dpi = 300)
...
ggsave("model/figures/model5/model5_1_crop_pixel_comparison_sim_obs_2018_2019.jpg",
       plot = p4,
       width = 14, height = 6, dpi = 300)
...
#crop_hist_sim
#<-read_csv("model/crop_hist_sim_model5.1.csv")
...
for (yr in predict_years) {
  ...
  ggsave(
    filename = paste0("model/figures/model5/model5_1_crop_choice_model_", yr, ".jpg"),
...
for (yr in predict_years) {
  ...
  ggsave(
    filename = paste0("model/figures/model5/model5_1_crop_planting_weeks_model_", yr, ".jpg"),
    plot = p, width = 8, height = 6, dpi = 300
  )
}
...
ggsave(filename = paste0("model/figures/model5/hist_crop_choice_", 2019, ".jpg"),
       plot = p, width = 8, height = 6, dpi = 300)
